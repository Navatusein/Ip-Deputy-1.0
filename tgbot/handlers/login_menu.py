...
async def login_contact(message: types.Message, state: FSMContext):
    ...
    if student is None:
        ...
        logger.info(f"id: {user_id} phone: {phone_number} user: {message.from_user.username} can't be find in the database")
        return
    ...
    logger.info(f"id: {user_id} phone: {phone_number} user: {message.from_user.username} authorized successfully")
    await state.finish()
